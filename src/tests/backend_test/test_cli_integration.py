...
def test_cli_help():
    ...
    result = subprocess.run([sys.executable, "-m", "src.backend.cli", "--help"], capture_output=True, text=True)
...
def test_curate_help():
    ...
    result = subprocess.run([sys.executable, "-m", "src.backend.cli", "curate", "--help"], capture_output=True, text=True)
...
def test_curate_status_no_login():
    ...
    result = subprocess.run([sys.executable, "-m", "src.backend.cli", "curate", "status"], capture_output=True, text=True)
...
def main():
    ...
    tests = [test_cli_help, test_curate_help, test_curate_status_no_login]
