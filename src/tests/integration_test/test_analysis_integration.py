...
class TestPythonOOPAnalysisIntegration:
    ...
    def test_python_oop_analysis_in_cli(
        ...
        with patch(
            "sys.argv", ["cli", "analyze", str(sample_python_project_zip)]
...
class TestCppOOPAnalysisIntegration:
    ...
    def test_cpp_oop_analysis_in_cli(
...
class TestCOOPAnalysisIntegration:
    ...
    def test_c_oop_analysis_in_cli(
        self, isolated_test_env, temp_session_file, sample_c_project_zip
    ):
...
class TestJavaOOPAnalysisIntegration:
    ...
    def test_java_oop_analysis_in_cli(
        self, isolated_test_env, temp_session_file, sample_java_project_zip
    ):
...
class TestResumeGeneratorIntegration:
    ...
    def test_resume_generation_in_cli(
        self, isolated_test_env, temp_session_file, sample_python_project_zip
    ):
    ...
    def test_resume_generation_with_no_content(
        self, isolated_test_env, temp_session_file, tmp_path
    ):
        ...
        with patch("sys.argv", ["cli", "analyze", str(minimal_zip)]), patch(
            "sys.stdout", new=StringIO()
        ) as fake_out:
            result = main()
            output = fake_out.getvalue()
...
class TestDatabaseStorageIntegration:
    ...
    def test_analysis_stored_in_database(
        self, isolated_test_env, temp_session_file, sample_python_project_zip
    ):
        ...
        with patch(
            ...
            with get_connection() as conn:
                cursor = conn.execute(
                    "SELECT COUNT(*) FROM analyses WHERE analysis_type = 'non_llm'"
                )
...
class TestLLMPipelineIntegration:
    ...
    def test_llm_command_requires_login(
        self, isolated_test_env, sample_python_project_zip
    ):
        """Test that LLM analysis requires login."""
        with patch(
            "sys.argv", ["cli", "analyze-llm", str(sample_python_project_zip)]
        ), patch("sys.stdout", new=StringIO()) as fake_out:
            result = main()
            output = fake_out.getvalue()
    ...
    def test_llm_analysis_requires_consent(
        self, isolated_test_env, temp_session_file, sample_python_project_zip
    ):
    ...
    def test_llm_analysis_requires_zip_file(
        self, isolated_test_env, temp_session_file, tmp_path
    ):
    ...
    def test_llm_analysis_with_mock_gemini(
        self, isolated_test_env, temp_session_file, sample_python_project_zip
    ):
    ...
    def test_llm_analysis_with_custom_prompt(
        self, isolated_test_env, temp_session_file, sample_python_project_zip
    ):
...
class TestCompleteAnalysisPipeline:
    ...
    def test_multi_language_project_analysis(
        ...
        with patch("sys.argv", ["cli", "analyze", str(multi_zip)]), patch(
            "sys.stdout", new=StringIO()
        ) as fake_out:
            result = main()
            output = fake_out.getvalue()
            ...
            assert (
                "python" in output_lower
                or "java" in output_lower
                or "cpp" in output_lower
            )
...
class TestErrorHandlingInIntegration:
    ...
    def test_analysis_handles_corrupted_zip(
        self, isolated_test_env, temp_session_file, tmp_path
    ):
        ...
        with patch("sys.argv", ["cli", "analyze", str(bad_zip)]), patch(
            "sys.stdout", new=StringIO()
        ) as fake_out:
            result = main()
            output = fake_out.getvalue()
    ...
    def test_analysis_handles_empty_zip(
        self, isolated_test_env, temp_session_file, tmp_path
    ):
        ...
        with patch("sys.argv", ["cli", "analyze", str(empty_zip)]), patch(
            "sys.stdout", new=StringIO()
        ) as fake_out:
            result = main()
            output = fake_out.getvalue()
