...
def _generate_detailed_git_contribution_summary(analysis: dict) -> str:
    """
    Generate detailed git contribution summary including:
    - semantic_summary (trivial vs substantial commits)
    - activity_breakdown (code/test/docs/design)
    - contribution_volume (lines changed)
    ...
    if target_user_email:
        ...
        if user_activity:
            ...
            if user_activity.get("code", 0) > 0:
                activity_parts.append(f"{user_activity['code']} code lines")
            if user_activity.get("test", 0) > 0:
                activity_parts.append(f"{user_activity['test']} test lines")
