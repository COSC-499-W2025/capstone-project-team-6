    ...
    def _detect_design_patterns(self):
        ...
        for class_name, class_info in self.classes.items():
            ...
            if "Singleton" in class_name or any(
                m['name'] in ('getInstance', 'instance') 
...
def calculate_oop_score(analysis: CppOOPAnalysis) -> int:
    ...
    if (analysis.private_fields > 0 or analysis.private_methods > 0 or
            analysis.protected_fields > 0 or analysis.protected_methods > 0):
        score += 1
    
    if (analysis.virtual_methods > 0 or
            analysis.override_methods > 0 or
            analysis.operator_overloads > 0):
        score += 1
    
    if (analysis.template_classes > 0 or
            analysis.namespaces_used > 0 or
            len(analysis.design_patterns) > 0):
        score += 1
