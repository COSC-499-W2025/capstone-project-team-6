        ...
        if project.get("target_user_email"):
            try:
                ...
                if score_data:
                    ...
                    print(
                        f"   Composite Score: {score_data.get('composite_score', 0):.2f}/100.0"
                    )
                    user_score = score_data.get("user_contribution_score", 0.0)
                    if user_score > 0:
                        print(f"   User Contribution Boost: {user_score:.2f}/20.0")
                        print(
                            f"   Adjusted Score: {score_data.get('adjusted_score', 0):.2f}"
                        )
                    ...
                    if breakdown:
                        ...
                        print(
                            f"      • Code Architecture: {breakdown.get('code_architecture', 0):.2f}/30.0"
                        )
                        print(
                            f"      • Code Quality: {breakdown.get('code_quality', 0):.2f}/25.0"
                        )
                        print(
                            f"      • Project Maturity: {breakdown.get('project_maturity', 0):.2f}/25.0"
                        )
                        print(
                            f"      • Algorithmic Quality: {breakdown.get('algorithmic_quality', 0):.2f}/20.0"
                        )
                    if justification.get("target_user"):
                        print(
                            f"   Contribution Justification: {justification['target_user']}"
                        )
            except Exception:
